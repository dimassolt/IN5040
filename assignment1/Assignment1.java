...
public class Assignment1 {
    private static final long SLEEP_TIME = 5; /* This variable determines the output rate of tuples sent to Esper to "simulate" a data stream. */
    ...
    public static void main(String[] args) throws InterruptedException, FileNotFoundException, EPCompileException, EPDeployException {
        Logger logger = LoggerFactory.getLogger(Assignment1.class); /* Set up the logger to avoid Esper warnings.. */

        EPCompiler compiler = EPCompilerProvider.getCompiler();
        Configuration configuration = new Configuration();
        configuration.getCommon().getTimeSource().setTimeUnit(TimeUnit.MILLISECONDS); /* Enable Esper to understand the timestamps from the .csv files. Hint: ext_timed..*/
        ...
        /* This loop will loop through a set of queries. Note that the corresponding .epl files are needed */
        ...
        EPStatement statement = runtime.getDeploymentService().getStatement(deployment.getDeploymentId(), "esper-statement");
        statement.addListener(new EventListener());

        /* Define the files that holds the data that we will use to simulate the input streams */
    ...
    public void run() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                tuple.setTimestamp(LocalDate.parse(tokenizer.nextToken()).atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000); //Esper expect milliseconds since epoch for timestamps!
                ...
                Thread.sleep(SLEEP_TIME); /* This will slow things a bit down, just to get a bit more "stream-like" behaviour from our program */
            }
            reader.close();
        } catch (Exception e) {
            e.printStackTrace();
